...
const globeMapURL = "/gh-land-sea-map.png";
...
image.src = globeMapURL;

const parentContainer = new THREE.Group();

// Always load globe with user visible
const euler = new THREE.Euler(0.3, 4.6, 0.05); // TODO understand
let rot = euler;
...
const shadowPoint = new THREE.Vector3()
    ...
    .add(new THREE.Vector3(0.7 * globeRadius, 0.3 * -globeRadius, globeRadius)),
  highlightPoint = new THREE.Vector3()
    .copy(parentContainer.position)
    .add(new THREE.Vector3(1.5 * -globeRadius, 1.5 * -globeRadius, 0)),
  frontPoint = new THREE.Vector3()
    .copy(parentContainer.position)
    .add(new THREE.Vector3(0, 0, globeRadius)),
  globe = new GLOBE({
    radius: globeRadius,
    detail: 55,
    renderer: renderer,
    shadowPoint: shadowPoint,
    shadowDist: 1.5 * globeRadius,
...
// HALO
const a = new THREE.SphereGeometry(globeRadius, 45, 45),
...
light_spot_1.position.set(
  parentContainer.position.x - 2.5 * globeRadius,
  80,
  -40
); //.multiplyScalar(t);
light_spot_2.position.set(
  parentContainer.position.x + globeRadius,
  globeRadius,
  2 * globeRadius
); //.multiplyScalar(t)
...
function draw_world_dots() {
  ...
  for (let lat = -90; lat <= 90; lat += 180 / r) {
    const radius = Math.cos(Math.abs(lat) * degToRad) * globeRadius;
    ...
    for (let r = 0; r < circum; r++) {
      ...
      const s = calc_pos(lat, lng, globeRadius);
      e.position.set(s.x, s.y, s.z);
      const o = calc_pos(lat, lng, globeRadius + 5);
